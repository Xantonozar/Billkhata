...
type Tab = 'Bill Payments' | 'Shopping' | 'Deposits' | 'Join Requests';
...
const PendingApprovalsPage: React.FC = () => {
    ...
    const handleAction = (category: keyof typeof initialApprovals, id: string) => {
        setApprovals(prev => ({
            ...prev,
    ...
    const totalTasks = Object.values(approvals).reduce((sum, arr) => sum + arr.length, 0);
                        ...
                        {approvals.billPayments.map(item => (
                            <div key={item.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-5">
                                ...
                                <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700 flex flex-wrap gap-2 justify-end">
                                    <button className="text-sm font-semibold text-primary hover:underline">View Bill Details</button>
                                    <button onClick={() => handleAction('billPayments', item.id)} className="px-3 py-1 text-sm font-semibold bg-red-100 text-red-700 rounded-md hover:bg-red-200 dark:bg-red-900/50 dark:text-red-300 dark:hover:bg-red-900">Deny</button>
                                    <button onClick={() => handleAction('billPayments', item.id)} className="px-3 py-1 text-sm font-semibold bg-green-100 text-green-700 rounded-md hover:bg-green-200 dark:bg-green-900/50 dark:text-green-300 dark:hover:bg-green-900">Approve Payment</button>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="space-y-4">
                        {approvals.shopping.map(item => (
                            <div key={item.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-5">
                                ...
                                <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700 flex flex-wrap gap-2 justify-end">
                                    <button onClick={() => handleAction('shopping', item.id)} className="px-3 py-1 text-sm font-semibold bg-red-100 text-red-700 rounded-md hover:bg-red-200 dark:bg-red-900/50 dark:text-red-300 dark:hover:bg-red-900">Deny with Reason</button>
                                    <button onClick={() => handleAction('shopping', item.id)} className="px-3 py-1 text-sm font-semibold bg-green-100 text-green-700 rounded-md hover:bg-green-200 dark:bg-green-900/50 dark:text-green-300 dark:hover:bg-green-900">Approve</button>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="space-y-4">
                        {approvals.deposits.map(item => (
                             <div key={item.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-5">
                                ...
                                <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700 flex flex-wrap gap-2 justify-end">
                                    <button onClick={() => handleAction('deposits', item.id)} className="px-3 py-1 text-sm font-semibold bg-red-100 text-red-700 rounded-md hover:bg-red-200 dark:bg-red-900/50 dark:text-red-300 dark:hover:bg-red-900">Deny with Reason</button>
                                    <button onClick={() => handleAction('deposits', item.id)} className="px-3 py-1 text-sm font-semibold bg-green-100 text-green-700 rounded-md hover:bg-green-200 dark:bg-green-900/50 dark:text-green-300 dark:hover:bg-green-900">Approve</button>
                                </div>
                            </div>
                        ))}
                    </div>
